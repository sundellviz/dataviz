...
as well as best travel routes between the capitals. EDIT: I had to remove the background image
as it was eating too much memory and crashing the app.")),
...
server <- function(input, output, session){
  ...
  output$zerosumplot <- renderPlot({
    ...
    ggplot() +
      ...
      geom_sf(data = rv$hexcrop_merged, fill="#eeeeee", col="#555555", lwd=0.1) +
